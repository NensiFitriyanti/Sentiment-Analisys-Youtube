...
with st.form("add_video_form"):
    new_video = st.text_input("Tambah Video ID / URL YouTube:")
    submitted = st.form_submit_button("➕ Tambahkan")
    if submitted and new_video:
        if "youtube.com" in new_video or "youtu.be" in new_video:
            if "v=" in new_video:
                vid = new_video.split("v=")[1].split("&")[0]
            elif "youtu.be/" in new_video:
                vid = new_video.split("youtu.be/")[1].split("?")[0]
        else:
            vid = new_video
        if vid not in st.session_state.video_ids:
            st.session_state.video_ids.append(vid)
            save_video(vid)
        st.success(f"Video {vid} berhasil ditambahkan!")

# Tampilkan daftar video
st.write("🎬 Daftar Video yang Dianalisis:", st.session_state.video_ids)

# ================= AMBIL DAN ANALISIS KOMENTAR =================
all_data = []
summary = {}
